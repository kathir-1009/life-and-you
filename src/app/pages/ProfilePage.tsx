...
export function ProfilePage() {
  ...
  return (
    <div className="min-h-screen bg-cream pb-32 portal-context">
      ...
      <div className="bg-sage pt-12 pb-16 px-6 md:px-12 md:rounded-[40px] relative overflow-hidden text-center">
        {/* Abstract shapes */}
        <div className="absolute top-0 left-0 w-64 h-64 bg-white/5 rounded-full -translate-y-1/2 -translate-x-1/2 blur-3xl" />
        <div className="absolute bottom-0 right-0 w-48 h-48 bg-gold/10 rounded-full translate-y-1/2 translate-x-1/2 blur-2xl" />

        <div className="relative z-10 flex flex-col items-center">
          <div className="relative mb-6">
            <div className="w-24 h-24 bg-cream rounded-[32px] flex items-center justify-center text-sage-dark shadow-2xl border-4 border-white/20">
               <img src={user.avatar} alt="Profile" className="w-full h-full object-cover rounded-[28px]" />
            </div>
            <button className="absolute -bottom-2 -right-2 w-10 h-10 bg-gold rounded-2xl flex items-center justify-center border-4 border-sage shadow-lg text-sage-dark hover:scale-110 transition-all">
              <Edit2 size={16} />
            </button>
          </div>
      ...
      <div className="max-w-xl mx-auto px-4 sm:px-6 -mt-8 relative z-20 space-y-6">
        ...
        <div className="grid grid-cols-3 gap-4">
          {[
            { value: "12", label: "Sessions", color: "text-sage-dark" },
            { value: "8", label: "Streak", color: "text-gold" },
            { value: "24", label: "Items", color: "text-sage-dark" },
          ].map((stat, i) => (
            <div key={i} className="bg-white p-5 rounded-[28px] border border-sage/5 shadow-sm text-center">
              <div className={`text-2xl font-bold font-serif ${stat.color}`}>{stat.value}</div>
              <div className="text-[10px] font-bold text-sage-dark/40 uppercase tracking-widest mt-1">{stat.label}</div>
            </div>
          ))}
        </div>
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white rounded-[32px] overflow-hidden border border-sage/5 shadow-sm">
            <MenuItem icon={Settings} label="Global Settings" color="bg-sage" />
            <MenuItem icon={Bell} label="Push Notifications" color="bg-gold" badge="3" />
            <MenuItem icon={Lock} label="Security & Privacy" color="bg-sage" />
          </div>

          <SectionTitle>Support & Billing</SectionTitle>
          <div className="bg-white rounded-[32px] overflow-hidden border border-sage/5 shadow-sm">
            <MenuItem icon={CreditCard} label="Payment Methods" color="bg-[#B5C4BA]" />
            <MenuItem icon={HelpCircle} label="Help Center" color="bg-sage" />
          </div>
          ...
          <div className="pt-8 border-t border-sage/10">
             <button 
                ...
                className="w-full bg-sage/5 text-sage-dark py-4 rounded-2xl text-[10px] font-bold uppercase tracking-widest hover:bg-sage hover:text-white transition-all"
             >
  ...
  return (
    <button className="w-full flex items-center gap-4 p-5 hover:bg-[#FBF5E6] transition-all group active:opacity-70">
      <div className={`w-11 h-11 ${color} rounded-2xl flex items-center justify-center text-white shadow-lg group-hover:scale-105 transition-all`}>
        <Icon size={20} className={color === 'bg-[#B5C4BA]' ? 'text-sage-dark' : 'text-white'} />
      </div>
      <span className="flex-1 text-left text-xs font-black text-sage-dark uppercase tracking-widest">
        {label}
      </span>
      {badge && (
        <span className="bg-gold text-sage-dark text-[9px] font-bold px-2 py-0.5 rounded-full">{badge}</span>
      )}
      <ChevronRight size={18} className="text-sage-dark/20 group-hover:text-sage-dark group-hover:translate-x-1 transition-all" />
    </button>
  );
}
