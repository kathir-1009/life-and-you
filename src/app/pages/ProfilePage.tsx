    ...
    <div className="min-h-screen bg-[#FCF8E8] pb-32 portal-context">
      ...
      <div className="bg-[#99A88C] pt-12 pb-16 px-6 md:px-12 md:rounded-[40px] relative overflow-hidden text-center">
        ...
        <div className="relative z-10 flex flex-col items-center">
          <div className="relative mb-6">
            <div className="w-24 h-24 bg-[#FCF8E8] rounded-[32px] flex items-center justify-center text-[#5E6C54] shadow-2xl border-4 border-white/20">
               <img src={user.avatar} alt="Profile" className="w-full h-full object-cover rounded-[28px]" />
            </div>
            <button className="absolute -bottom-2 -right-2 w-10 h-10 bg-[#C4A35A] rounded-2xl flex items-center justify-center border-4 border-[#99A88C] shadow-lg text-[#5E6C54] hover:scale-110 transition-all">
              <Edit2 size={16} />
            </button>
          </div>
      ...
      <div className="max-w-xl mx-auto px-4 sm:px-6 -mt-8 relative z-20 space-y-6">
        ...
        <div className="grid grid-cols-3 gap-4">
          {[
            { value: "12", label: "Sessions", color: "text-[#5E6C54]" },
            { value: "8", label: "Streak", color: "text-[#C4A35A]" },
            { value: "24", label: "Items", color: "text-[#5E6C54]" },
          ].map((stat, i) => (
            <div key={i} className="bg-white p-5 rounded-[28px] border border-[#99A88C]/5 shadow-sm text-center">
              ...
              <div className="text-[10px] font-bold text-[#5E6C54]/40 uppercase tracking-widest mt-1">{stat.label}</div>
            </div>
          ))}
        </div>
        ...
        <div className="space-y-4">
          ...
          <div className="pt-8 border-t border-[#99A88C]/10">
             <button 
                ...
                className="w-full bg-[#99A88C]/5 text-[#5E6C54] py-4 rounded-2xl text-[10px] font-bold uppercase tracking-widest hover:bg-[#99A88C] hover:text-white transition-all"
             >
...
function SectionTitle({ children }: { children: React.ReactNode }) {
  return (
    <h3 className="text-[10px] font-black text-[#5E6C54]/60 uppercase tracking-[0.25em] px-4 pt-2">
      {children}
    </h3>
  );
}
  ...
  return (
    <button className="w-full flex items-center gap-4 p-5 hover:bg-[#FBF5E6] transition-all group active:opacity-70">
      <div className={`w-11 h-11 ${color} rounded-2xl flex items-center justify-center text-white shadow-lg group-hover:scale-105 transition-all`}>
        <Icon size={20} className={color === 'bg-[#B5C4BA]' ? 'text-[#5E6C54]' : 'text-white'} />
      </div>
      <span className="flex-1 text-left text-xs font-black text-[#5E6C54] uppercase tracking-widest">
        {label}
      </span>
      {badge && (
        <span className="bg-[#C4A35A] text-[#5E6C54] text-[9px] font-bold px-2 py-0.5 rounded-full">{badge}</span>
      )}
      <ChevronRight size={18} className="text-[#5E6C54]/20 group-hover:text-[#5E6C54] group-hover:translate-x-1 transition-all" />
    </button>
  );
}
