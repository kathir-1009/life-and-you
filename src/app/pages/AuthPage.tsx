    ...
    <div className="min-h-[100dvh] bg-[#FCF8E8] flex items-center justify-center px-6 py-12 portal-context">
      ...
      <div className="max-w-md w-full relative z-10">
        ...
        <div className="flex items-center justify-between mb-12">
           <Link to="/" className="p-3 bg-white rounded-2xl border border-[#99A88C]/10 text-[#5E6C54] shadow-sm hover:bg-[#EDF2EE] transition-colors">
              <ChevronLeft size={20} />
           </Link>
        ...
        <div className="bg-white/70 backdrop-blur-xl border border-white p-8 md:p-10 rounded-[40px] shadow-2xl">
          <div className="text-center mb-10">
            ...
            <h1 className="text-3xl font-bold text-[#5E6C54] font-serif mb-2">
              {mode === "login" ? "Welcome Back" : "Start Journey"}
            </h1>
            <p className="text-[#5E6C54]/60 text-sm font-medium">
              {mode === "login" 
          ...
          <div className="grid grid-cols-2 gap-4 mb-8">
             <button className="flex items-center justify-center gap-3 py-3 px-4 bg-white border border-[#99A88C]/10 rounded-2xl text-[10px] font-bold uppercase tracking-widest hover:bg-[#EDF2EE] transition-all">
                <Chrome size={16} className="text-[#5E6C54]" />
                Google
             </button>
          ...
          <div className="relative mb-8 flex items-center">
             ...
             <span className="mx-4 text-[10px] font-bold text-[#5E6C54]/30 uppercase tracking-widest">Or with email</span>
             <div className="flex-1 border-t border-[#99A88C]/10"></div>
          </div>
          ...
          <form className="space-y-4" onSubmit={handleAuth}>
            {mode === "register" && (
              <div className="space-y-1">
                 <label className="text-[10px] font-bold text-[#5E6C54]/40 uppercase tracking-widest ml-2">Display Name (Optional)</label>
                 <div className="relative">
                   <User className="absolute left-4 top-1/2 -translate-y-1/2 text-[#5E6C54]/30" size={18} />
                   <input 
                     type="text" 
            ...
            <div className="space-y-1">
               <label className="text-[10px] font-bold text-[#5E6C54]/40 uppercase tracking-widest ml-2">Email Identity (Optional for testing)</label>
               <div className="relative">
                 <Mail className="absolute left-4 top-1/2 -translate-y-1/2 text-[#5E6C54]/30" size={18} />
                 <input 
                   type="email" 
            ...
            <div className="space-y-1">
               <label className="text-[10px] font-bold text-[#5E6C54]/40 uppercase tracking-widest ml-2">Access Key (Optional)</label>
               <div className="relative">
                 <Lock className="absolute left-4 top-1/2 -translate-y-1/2 text-[#5E6C54]/30" size={18} />
                 <input 
                   type="password" 
          ...
          <div className="mt-8 pt-8 border-t border-[#99A88C]/5 flex justify-center gap-4">
             <button onClick={() => setRole('client')} className={`text-[9px] font-bold uppercase tracking-widest px-3 py-1 rounded-lg ${role === 'client' ? 'bg-[#99A88C] text-white' : 'text-[#5E6C54]/50'}`}>Patient</button>
             <button onClick={() => setRole('coach')} className={`text-[9px] font-bold uppercase tracking-widest px-3 py-1 rounded-lg ${role === 'coach' ? 'bg-[#99A88C] text-white' : 'text-[#5E6C54]/50'}`}>Doctor</button>
             <button onClick={() => setRole('admin')} className={`text-[9px] font-bold uppercase tracking-widest px-3 py-1 rounded-lg ${role === 'admin' ? 'bg-[#99A88C] text-white' : 'text-[#5E6C54]/50'}`}>Admin</button>
          </div>

          <div className="mt-6 text-center text-xs font-medium text-[#5E6C54]/60">
        ...
        <p className="mt-8 text-center text-[10px] text-[#5E6C54]/30 font-bold uppercase tracking-[0.2em] flex items-center justify-center gap-2">
           <Lock size={12} />
