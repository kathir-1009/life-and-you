    ...
    <div className="min-h-screen bg-cream pb-32 portal-context">
      ...
      <div className="lg:hidden bg-[#2D3324] text-white px-6 py-6 rounded-b-[40px] relative overflow-hidden flex items-center gap-4 shadow-xl z-20">
         ...
         <div className="relative z-10 flex-1">
            <h1 className="text-xl font-black tracking-tight leading-none text-[#FFFFFF]">Session Details</h1>
            <p className="text-[#8B9A71] text-[9px] font-black uppercase tracking-[0.2em] mt-1">Deep dive into your progress</p>
         </div>
      </div>
      ...
      <div className="max-w-4xl mx-auto px-6 pt-16">
        ...
        <div className="grid lg:grid-cols-12 gap-12 items-start">
           ...
           <div className="lg:col-span-8 space-y-8">
              <div className="bg-white p-10 rounded-[48px] border border-sage/10 shadow-sm relative overflow-hidden">
                 <div className="absolute top-0 right-0 w-32 h-32 bg-sage/5 rounded-bl-[100px] -z-10" />
                 
                 <div className="flex items-center gap-4 mb-8">
                    <div className="px-4 py-1.5 bg-cream text-sage rounded-full text-[10px] font-black uppercase tracking-widest border border-sage/10">
                       Scheduled Session
                    </div>
                 ...
                 <h1 className="text-4xl font-bold text-sage-dark font-serif mb-6 leading-tight uppercase">Anxiety Breakthrough Session</h1>
                 <p className="text-sage-dark/60 font-medium leading-relaxed mb-12 italic text-lg">
              ...
              <div className="bg-white p-10 rounded-[40px] border border-sage/10 shadow-sm">
                 <h3 className="text-xl font-bold text-sage-dark font-serif mb-8 uppercase tracking-tight">Preparation Notes</h3>
                 <div className="space-y-4">
                    <label className="flex gap-4 items-start p-4 bg-cream/50 rounded-2xl">
                       <input type="checkbox" className="mt-1" />
                       <span className="text-sm text-sage-dark/80 font-medium">Find a quiet, private space where you won't be interrupted.</span>
                    </label>
                    <label className="flex gap-4 items-start p-4 bg-cream/50 rounded-2xl">
                       <input type="checkbox" className="mt-1" />
                       <span className="text-sm text-sage-dark/80 font-medium">Have your journal and a glass of water nearby.</span>
                    </label>
                    <label className="flex gap-4 items-start p-4 bg-cream/50 rounded-2xl">
                       <input type="checkbox" className="mt-1" />
                       <span className="text-sm text-sage-dark/80 font-medium">Reflect on your emotional state for 5 minutes before joining.</span>
                    </label>
                 </div>
              </div>
           </div>
           ...
           <div className="lg:col-span-4 space-y-6 lg:sticky lg:top-36">
              <Link 
                to={`/session/${id}`}
                className="w-full py-6 bg-sage-dark text-white rounded-full font-bold text-xs uppercase tracking-widest shadow-2xl hover:scale-[1.03] transition-all flex items-center justify-center gap-3"
              >
              ...
              <div className="bg-sage-dark p-8 rounded-[40px] text-white shadow-xl group cursor-pointer hover:bg-[#4A5D4E] transition-all">
                 <div className="w-12 h-12 bg-white/10 rounded-2xl flex items-center justify-center mb-6">
                    <FileEarmarkTextFill size={24} className="text-gold-dark" />
                 </div>
                 ...
                 <div className="flex gap-1">
                    {[1,2,3].map(i => <div key={i} className="w-1.5 h-1.5 rounded-full bg-gold-dark" />)}
                 </div>
              </div>
           </div>
        </div>
...
function DetailItem({ icon: Icon, label, val }: { icon: any, label: string, val: string }) {
  return (
    <div className="space-y-2">
       ...
       <p className="text-sm font-bold text-sage-dark tracking-tight">{val}</p>
    </div>
  );
}
...
function ActionButton({ to, icon: Icon, label, color }: { to: string, icon: any, label: string, color: string }) {
  return (
    <Link to={to} className={`bg-white p-6 rounded-[32px] border border-sage/10 flex flex-col items-center gap-3 hover:shadow-lg transition-all shadow-sm`}>
