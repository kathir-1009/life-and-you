  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-5 duration-700 px-4 md:px-0 portal-context pb-20">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
         {coaches.map((coach) => (
           <div key={coach.id} className="bg-white rounded-[40px] p-6 lg:p-8 border border-sage/5 shadow-sm hover:shadow-xl transition-all group flex flex-col items-center text-center">
              <div className="relative mb-8">
                 <div className="w-24 h-24 bg-cream rounded-[32px] flex items-center justify-center text-sage shadow-sm font-serif text-3xl font-bold group-hover:scale-110 transition-transform">
                    {coach.img}
                 </div>
                 <div className="absolute -bottom-2 -right-2 w-8 h-8 bg-sage rounded-full border-4 border-white flex items-center justify-center text-white shadow-lg">
                    <PatchCheckFill size={14} />
                 </div>
              </div>

              <div className="mb-6">
                 <h3 className="text-xl font-bold text-sage-dark font-serif mb-1 uppercase tracking-tight">{coach.name}</h3>
                 <p className="text-[9px] text-gold-dark font-black uppercase tracking-[0.2em] mb-3">{coach.role}</p>
                 <div className="flex items-center justify-center gap-1.5">
                    <StarFill size={10} className="text-gold-dark" />
                    <span className="text-[10px] font-bold text-sage-dark">{coach.rating}</span>
                    <span className="text-[9px] font-bold text-sage-dark/30">({coach.reviews} reviews)</span>
                 </div>
              </div>

              <div className="w-full bg-cream/50 p-4 rounded-2xl mb-8">
                 <p className="text-[10px] text-sage-dark/60 font-bold uppercase tracking-widest mb-1.5">Primary Focus</p>
                 <p className="text-xs font-black text-sage-dark uppercase tracking-tight">{coach.specialization}</p>
              </div>
              ...
              <Link 
                ...
                className="w-full py-4 bg-sage-dark text-white rounded-2xl text-[10px] font-black uppercase tracking-widest shadow-lg shadow-sage-dark/10 hover:bg-[#4A5D4E] transition-all flex items-center justify-center gap-3"
              >
...
function ChannelIcon({ icon: Icon }: { icon: any }) {
  return (
    <div className="w-10 h-10 bg-white border border-sage/10 rounded-xl flex items-center justify-center text-sage/40">
       <Icon size={16} />
    </div>
  );
}
