...
export function CoachRegisterPage() {
  ...
  const handleAuth = (e: React.FormEvent) => {
    e.preventDefault();
    navigate("/auth/verify", { state: { mode: "register" } });
  };
      ...
      <div className="max-w-md w-full relative z-10">
        ...
        <div className="bg-white p-8 md:p-10 rounded-[40px] shadow-2xl">
          <div className="text-center mb-10">
            <h1 className="text-3xl font-bold text-[#2D3324] font-serif mb-2">Join the Circle</h1>
            <p className="text-[#2D3324]/50 text-sm font-medium">Apply to become a mentor and guide others</p>
          </div>
          ...
          <form className="space-y-4" onSubmit={handleAuth}>
            <div className="space-y-1">
              <label className="text-[10px] font-bold text-[#2D3324]/50 uppercase tracking-widest ml-2">Full Name</label>
              <div className="relative">
                <User className="absolute left-4 top-1/2 -translate-y-1/2 text-[#2D3324]/30 pointer-events-none" size={18} />
                <input type="text" required placeholder="Your professional name" className="w-full bg-[#F3F5F0] border border-[#8B9A71]/20 rounded-2xl pl-12 pr-4 py-4 text-sm text-[#2D3324] outline-none focus:border-[#8B9A71] focus:bg-white transition-[background-color,border-color] duration-200 placeholder:text-[#2D3324]/30" />
              </div>
            </div>
            ...
            <div className="space-y-1">
              ...
              <div className="relative">
                ...
                <input type="password" required placeholder="Create a secure password" className="w-full bg-[#F3F5F0] border border-[#8B9A71]/20 rounded-2xl pl-12 pr-4 py-4 text-sm text-[#2D3324] outline-none focus:border-[#8B9A71] focus:bg-white transition-[background-color,border-color] duration-200 placeholder:text-[#2D3324]/30" />
              </div>
            </div>
            ...
            <div className="mt-4 text-center text-[10px] font-medium text-[#2D3324]/50">
              ...
              <Link to="/auth/coach/login" className="text-[#8B9A71] font-bold hover:text-[#2D3324] transition-colors uppercase tracking-widest">
                Sign In
              </Link>
            </div>
          </form>
