...
export function ProgressPage() {
  ...
  return (
    <div className="min-h-screen bg-[#F5EFE6] pb-32 portal-context">
      {/* Cinematic Header */}
      <div className="bg-[#1C2320] pt-12 pb-24 lg:pb-32 px-6 lg:px-10 md:rounded-b-[80px] relative overflow-hidden">
      ...
      <div className="max-w-6xl mx-auto px-4 lg:px-10 -mt-12 lg:-mt-20 relative z-20">
         <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
            ...
            <div className="lg:col-span-8 space-y-8 text-center lg:text-left">
               <div className="bg-white rounded-[44px] p-8 lg:p-12 shadow-2xl border border-[#3D5247]/5">
                  <div className="flex items-center justify-between mb-12">
                     <h3 className="text-xl lg:text-2xl font-bold text-[#1C2320] font-serif">Weekly Emotional Rhythm</h3>
                  ...
                  <div className="flex items-end justify-between h-56 lg:h-80 gap-3">
                     {weekData.map((data, i) => (
                        <div key={i} className="flex-1 flex flex-col items-center gap-6 group justify-end h-full">
                           <div 
                              className={`w-full max-w-[48px] rounded-full transition-all duration-1000 relative flex justify-center items-center group shadow-md ${i === 4 ? 'bg-[#1C2320]' : 'bg-[#3D5247] hover:bg-[#A68A45]'}`}
                              style={{ height: `${data.height}%` }}
                           >
                              <div className="absolute -top-10 opacity-0 group-hover:opacity-100 transition-all bg-[#1C2320] text-white text-[10px] font-black px-2 py-1 rounded-lg">
                                 {data.val}
                              </div>
                           </div>
                           <span className="text-[10px] lg:text-xs font-black text-[#1C2320]/40 uppercase tracking-widest">{data.day}</span>
                        </div>
                     ))}
                  </div>
               </div>
               ...
               <div className="space-y-4">
                  <h3 className="text-xl font-bold text-[#1C2320] font-serif px-2">Focus Progress</h3>
                  <div className="grid md:grid-cols-2 gap-4">
                     {[
                        { label: "Anxiety Control", val: 82, icon: Shield, color: "bg-[#3D5247]" },
                        { label: "Sleep Pattern", val: 65, icon: Calendar, color: "bg-[#1C2320]" },
                        { label: "Stability", val: 90, icon: Heart, color: "bg-[#A68A45]" },
                        { label: "Communication", val: 74, icon: TrendingUp, color: "bg-[#3D5247]" },
                     ].map((item, i) => (
                        <div key={i} className="bg-white p-6 rounded-[32px] border border-[#3D5247]/5 flex items-center gap-5 hover:shadow-xl transition-all group shadow-sm">
                           ...
                           <div className="flex-1">
                             <div className="flex justify-between items-center mb-3">
                                <span className="text-sm font-black text-[#1C2320] uppercase tracking-tight">{item.label}</span>
            ...
            <div className="lg:col-span-4 space-y-6 lg:sticky lg:top-36 text-center lg:text-left">
               <div className="bg-[#1C2320] rounded-[44px] p-8 lg:p-10 text-white overflow-hidden relative group shadow-2xl">
                  ...
                  <div className="flex flex-col items-center lg:items-start relative z-10">
                     <div className="w-16 h-16 bg-[#A68A45] rounded-[24px] flex items-center justify-center text-[#1C2320] shadow-2xl mb-8 group-hover:scale-110 transition-transform">
                        <Award size={36} />
                     </div>
               ...
               <div className="bg-[#A68A45] p-8 lg:p-10 rounded-[44px] group cursor-pointer hover:scale-[1.02] transition-all shadow-xl">
                  <h4 className="text-[10px] font-black text-[#1C2320] uppercase tracking-[0.3em] mb-6 opacity-60">Upcoming Milestone</h4>
                  <div className="flex items-center justify-between gap-4">
                     <p className="text-xl font-bold text-[#1C2320] font-serif leading-tight max-w-[160px]">Unlock Advanced NLP Journaling</p>
                     <div className="w-12 h-12 bg-[#1C2320] text-white rounded-2xl flex items-center justify-center group-hover:translate-x-2 transition-transform shadow-lg">
                        <ArrowRight size={20} />
                     </div>
                  </div>
                  <p className="text-[10px] font-black text-[#1C2320]/60 uppercase tracking-widest mt-6">Complete 3 more sessions</p>
               </div>
            </div>
         </div>
      </div>
