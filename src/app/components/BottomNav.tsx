...
export function BottomNav() {
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 bg-white/10 backdrop-blur-2xl border-t border-white/10 px-6 py-3 pb-8 md:pb-4 flex items-center justify-between z-50 rounded-t-[32px] shadow-[0_-10px_40px_rgba(0,0,0,0.1)] md:max-w-md md:mx-auto md:bottom-6 md:rounded-full md:border">
      {navItems.map((item) => {
        ...
        return (
          ...
          >
            <div className={`w-12 h-12 rounded-2xl flex items-center justify-center transition-all duration-300 ${active ? 'bg-[#8B9A71] text-white shadow-lg shadow-[#8B9A71]/20 scale-110 -translate-y-1' : 'text-[#2D3324]/40 hover:text-[#8B9A71]'}`}>
