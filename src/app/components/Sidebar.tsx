...
export function Sidebar() {
  ...
  return (
    <div className="w-80 h-screen sticky top-0 bg-[#1C2320] flex flex-col p-8 border-r border-[#3D5247]/20 shadow-2xl overflow-y-auto hidden lg:flex custom-scrollbar">
      {/* Background Decor */}
      <div className="absolute top-0 left-0 w-64 h-64 bg-[#3D5247]/5 rounded-full -translate-y-1/2 -translate-x-1/2 blur-[80px]" />
          ...
          {role === 'client' && (
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                 <div className={`p-1.5 rounded-lg ${isAnonymous ? 'bg-[#3D5247] text-white' : 'bg-white/10 text-white/40'}`}>
              ...
              <button 
                ...
                className={`w-9 h-5 rounded-full relative transition-all ${isAnonymous ? 'bg-[#3D5247]' : 'bg-white/10'}`}
              >
          ...
          return (
            <Link
              ...
              className={`flex items-center gap-4 px-5 py-3.5 rounded-[16px] transition-all group ${
                active 
                  ? "bg-[#3D5247] text-white shadow-lg shadow-black/20" 
            ...
            >
              ...
              {item.badge && (
                <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded ${active ? 'bg-white/20 text-white' : 'bg-[#C4A35A] text-[#1C2320]'}`}>{item.badge}</span>
              )}
            </Link>
          );
        })}
