    ...
    <div className="bg-white p-5 rounded-[24px] border border-[#99A88C]/10 flex flex-col sm:flex-row sm:items-center justify-between gap-4 group hover:border-[#99A88C]/30 transition-all">
      <div className="flex items-center gap-4">
        <div className={`w-14 h-14 rounded-2xl flex flex-col items-center justify-center shrink-0 ${
          session.status === 'completed' ? 'bg-[#EDF2EE] text-[#5E6C54]' : 'bg-[#99A88C] text-white'
        }`}>
        ...
        <div>
          <div className="flex items-center gap-2 mb-1">
            <h4 className="font-bold text-[#5E6C54] text-sm">
              {role === 'client' ? `Coach ${session.participantName}` : session.participantName}
            </h4>
            {session.isAnonymous && (
              <span className="text-[9px] font-bold bg-[#EDF2EE] text-[#5E6C54] px-1.5 py-0.5 rounded-md uppercase tracking-wider">Anon</span>
            )}
          </div>
      ...
      <div className="flex items-center gap-3">
        {isUpcoming ? (
          <>
            ...
            <button className="p-2.5 text-[#5E6C54]/40 hover:text-[#5E6C54] transition-colors">
